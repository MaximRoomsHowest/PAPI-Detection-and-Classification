...
Compose. Source layout and architecture are covered in the architecture overview.
...
calibration was not available), so zoom-regime numbers are reported as absent — not zero
and not invented.
...
- The 10 fps real-time target is therefore met on GPU-class hardware with margin and is not
  met CPU-only at 1280 px. The client workstation (WL051) specs were not available, so edge
...
| Item | Status |
| --- | --- |
| Day-zoom regime | Not evaluated (flight absent from twin; `calibrated_focal_px` unavailable) |
| Edge hardware numbers | WL051 hardware unavailable during the project; laptop reference only |
...
| Adverse-weather robustness | Serving yolo26s collapses under snow (≈0.03); weather-robust `nano-weather` exists (≈0.82, MODELS.md §3.2b); yolo26s weather retrain is future work |
| Rwy-06 commissioned set-angles | Not provided by client (FAA defaults in config; flagged per analysis) |
