...
Inside every promoted run folder the deployable checkpoint keeps Ultralytics'
conventional `weights/best.pt` name. The repo no longer commits
`weights/last.pt`, preview batches, or validation plot dumps; those are
training-workspace artifacts that can be regenerated from the scripts and
metadata. Only the **folder** carries the descriptive model name.
...
| `modeltransfered` | `yolov8s-transfer` *(archived outside the lean handover repo)* |
...
**INT8 — retired.** The old INT8 export was quantised from the RETIRED yolo26n
model and failed on CPU ORT (`ConvInteger(10)` not implemented). It is no
longer shipped in the lean handover repo. A
...
| `yolov8s-transfer` | yolov8s | transfer | archived outside repo | Legacy YOLOv8 transfer seed; comparison only |
| `val-5…7`, `val-12…15` | yolo26s | validation passes | regenerated by evaluation workflows | PR/F1 curves + confusion matrices, no weights; not shipped in the lean repo |
...
- INT8 path: re-export from the new serving model (§3.2.1) once an edge
  target is fixed; the old retired export was CPU-broken and is no longer
  shipped.
